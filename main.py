...
from brand_floor import apply_brand_floor
...
def valuate(req: ValuationRequest):
    try:
        ...
        # ── Brand floor correction ────────────────────────────────
        # Post-processing layer: encodes domain knowledge where
        # Mercari training data structurally cannot capture market reality.
        # Ultra-high: full Model A override. High: soft floor blend.
        # Contemporary: passthrough (XGBoost unmodified).
        floor = apply_brand_floor(
            xgboost_price=price,
            xgboost_price_low=price_low,
            xgboost_price_high=price_high,
            brand=brand_norm,
            brand_tier=brand_tier,
            condition_score=condition_score,
            original_retail_price=req.original_retail_price,
            category=req.category,
            has_accessories=bool(parsed["accessories_flag"]),
            size=req.size,
        )
        price = floor.corrected_price
        price_low = floor.confidence_low
        price_high = floor.confidence_high
        ...
        rationale = {
            ...
            # Brand floor correction fields
            "floor_correction_applied": floor.floor_applied,
            "floor_logic": floor.floor_logic,
            "retention_rate": floor.retention_rate if floor.floor_applied else None,
            "market_base": floor.market_base if floor.floor_applied else None,
        }
